...
class ChessSquare(models.Model):
    board = ParentalKey(
        'ChessBoardSnippet',
        on_delete=models.CASCADE,
        related_name="squares"
    )
    name = models.CharField(max_length=255, help_text="The name displayed on this square.")
    position = models.CharField(
        max_length=2,
        help_text="Position of the square (e.g., A1, B2). This determines where it appears on the board."
    )
    sponsoring_since = models.DateField(blank=True, null=True)
    picture = models.ImageField(upload_to="sponsor_pictures/", blank=True, null=True)
    website = models.URLField(blank=True, null=True)

    def __str__(self):
        return f"{self.position}: {self.name or 'Empty'}"


@register_snippet
class ChessBoardSnippet(ClusterableModel):
    """A snippet to hold all chess squares."""
    name = models.CharField(max_length=255, unique=True, help_text="Name of this sponsor board.")

    panels = [
        FieldPanel("name"),
        InlinePanel("squares", label="Squares"),
    ]
